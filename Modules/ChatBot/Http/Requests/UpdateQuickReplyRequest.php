...
class UpdateQuickReplyRequest extends FormRequest
{
    ...
    {
        return $this->user() && $this->user()->can('update quick replies');
    }
    ...
    {
        $id = $this->route('quickReply')?->id;

        return [
            'shortcut' => ['required', 'string', 'max:50', 'regex:/^[a-zA-Z0-9_-]+$/', "unique:quick_replies,shortcut,{$id}"],
